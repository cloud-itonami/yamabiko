(ns yamabiko.methods.social
  "social.cljc — 山彦 — High-Speed Rail Manufacturing DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from yamabiko's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "山彦 — High-Speed Rail Manufacturing が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over yamabiko's own domain (High-speed rail trainset manufacturing Tier-B actor. Wave 1 reference = civilian Shinkansen-class trainset (~250-320 km/h, 8-16 cars).…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "yamabiko R0: live social posting is Council Lv6+ + operator + "
